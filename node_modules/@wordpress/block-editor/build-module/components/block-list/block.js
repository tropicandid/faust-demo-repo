...
function BlockListBlock(_ref2) {
  ...
  const {
    ...
    return {
      ...
      isContentBlock: select(blocksStore).__experimentalHasContentRoleAttribute(name),
  ...
  return {
    ...
    onMerge(forward) {
      ...
      const {
        ...
        getBlockName,
        getBlockOrder
      } = registry.select(blockEditorStore); // For `Delete` or forward merge, we should do the exact same thing
